...
public class StockIssuanceDAO {
                ...
                try (ResultSet rs = checkStmt.executeQuery()) {
                    ...
                    if (quantityRequested > available) {
                        ...
                        throw new InsufficientStockException(
                                "Cannot issue " + quantityRequested + " units of \"" + materialName +
    ...
    /** Full issuance history, most recent first, joined with material/cleaner names for display. */
    public List<StockIssuance> getIssuanceHistory() throws SQLException {
        String sql = "SELECT si.issuance_id, si.material_id, m.name AS material_name, " +
                "si.cleaner_id, c.first_name, c.last_name, si.quantity_issued, si.issued_date, si.issued_by " +
    ...
    /** Inventory report: every material with its current stock level. */
    public List<Material> getInventoryReport() throws SQLException {
        String sql = "SELECT material_id, name, quantity, reorder_level, unit FROM materials ORDER BY name";
    ...
    /** Low-stock report: only materials at or below their reorder level. */
    public List<Material> getLowStockReport() throws SQLException {
        String sql = "SELECT material_id, name, quantity, reorder_level, unit " +
                "FROM materials WHERE quantity <= reorder_level ORDER BY quantity ASC";
        List<Material> materials = new ArrayList<>();
    ...
    /** Material usage report: total quantity issued per material, most-used first. */
    public List<Object[]> getMaterialUsageReport() throws SQLException {
        String sql = "SELECT m.name, SUM(si.quantity_issued) AS total_issued " +
                "FROM stock_issuance si JOIN materials m ON si.material_id = m.material_id " +
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            while (rs.next()) {
                usage.add(new Object[]{rs.getString("name"), rs.getInt("total_issued")});
            }
        }
        return usage;
    }

    /** Used to populate the material dropdown on the issue-stock form. */
    ...
    /** Used to populate the cleaner dropdown on the issue-stock form. */
    public List<Cleaner> getAllCleaners() throws SQLException {
        String sql = "SELECT cleaner_id, first_name, last_name, department FROM cleaners ORDER BY first_name";
